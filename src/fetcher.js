...
const { decodePoll } = require('./pollDecoder');
...
async function fetchPollAccounts(rpcUrl = config.SOLANA_RPC_URL) {
  const body = {
    jsonrpc: '2.0',
    id: 1,
    method: 'getProgramAccounts',
    params: [
      config.PROGRAM_ID,
      {
        encoding: 'base64',
        filters: [
          { memcmp: { offset: 0, bytes: config.POLL_MEMCMP_BYTES } },
          { dataSize: config.POLL_DATA_SIZE },
        ],
      },
    ],
  };

  logger.debug({ rpcUrl, method: 'getProgramAccounts' }, 'Fetching poll accounts');
  ...
  const polls = (json.result || [])
    .map(({ pubkey, account }) => decodePoll(pubkey, account.data[0]))
    .filter(Boolean)
    .sort((a, b) => b.timestamp - a.timestamp);

  logger.info({ count: polls.length }, 'Fetched and decoded poll accounts');
  return polls;
}

module.exports = { fetchContentAccounts, fetchPollAccounts };
