...
3. **`GET /test-check?url=<mirror-link>&spoiler=<true|false>`** - Live test harness (only active when `TEST_DISCORD_WEBHOOK_URL` is set): wraps `url` in a fake leak account (one mirror item, e.g. a bedrive.ru/temp.sh/upload.ee/arweave link) and runs it through the **actual production `sendDiscordAlert`** (`src/notifier.js`) — same rich embed, same `resolveDirectVideos`/Filebase-mirroring video follow-up — just pointed at `TEST_DISCORD_WEBHOOK_URL` instead of the real `DISCORD_WEBHOOK_URL` channel. If `TEST_ENDPOINT_TOKEN` is set, requests must include a matching `?token=` param, since this endpoint triggers real network fetches/downloads/uploads/webhook posts. The optional `spoiler` param forces the spoiler embed rendering on/off for testing that path, overriding the real `SPOILER_KEYWORDS` title-matching logic.
...
- Some mirror hosts (e.g. `temp.sh`) serve an HTML "confirm download" page (a `<form method="POST">Click here to download</form>`) instead of the file on a plain GET. `src/resolver.js`'s `extractPostDownloadUrl` detects this pattern generically (not hardcoded per host) and both `resolveDirectVideo` and `mirrorVideoToFilebase` retry with a POST to actually fetch the real file. Content-Type is always verified to start with `video/` before anything is uploaded to Filebase — never assumed from a URL's file extension — since Filebase's free plan outright rejects HTML uploads.
